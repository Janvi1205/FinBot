...
def ask_stream():
    ...
    # 2️⃣ Financial relevance check (fuzzy)
        ...
        # For streaming, send error message as a single chunk
        error_prompt = (
            "The user asked: '{user_prompt}'\n\n"
            "CRITICAL LANGUAGE REQUIREMENT:\n"
            "{language_instruction}\n"
        ...
        def generate_error_response():
            try:
                error_response = model.generate_content(error_prompt, stream=True)
                ...
                # Fallback to English if error occurs
                error_text = "I can answer only questions related to Indian financial literacy — like budgeting, saving, investing, RBI rules, loans, tax, and government schemes.\n\nPlease ask something from these topics."
                for word in error_text.split():
                    yield f"data: {json.dumps({'chunk': word + ' '})}\n\n"
                yield f"data: {json.dumps({'done': True})}\n\n"
        
        response_obj = Response(stream_with_context(generate_error_response()), mimetype='text/event-stream')
        response_obj.headers['Cache-Control'] = 'no-cache'
        response_obj.headers['X-Accel-Buffering'] = 'no'
        return response_obj
    ...
    response_obj = Response(stream_with_context(generate_stream()), mimetype='text/event-stream')
    response_obj.headers['Cache-Control'] = 'no-cache'
    response_obj.headers['X-Accel-Buffering'] = 'no'
    return response_obj
